...
OUT_BASE="$ROOT/outputs/revision_e15b_efficiency_profile_20260803"
LOG_DIR="$ROOT/logs/revision_e15b_efficiency_profile_20260803"
...
if [[ ! -f "$OUT_BASE/mirage_assay_cold_s42/summary.csv" ]]; then
  /usr/bin/time -v -o "$LOG_DIR/mirage_assay_cold_s42.time.txt" \
    env CUDA_VISIBLE_DEVICES=6 "$PY" scripts/run_chembl_assay_v2_eval.py \
      --benchmark-dir "$BENCH" \
...
"$PY" scripts/build_revision_efficiency_snapshot.py \
  ...
  --output-dir outputs/revision_e15b_efficiency_profile_20260803/snapshot

echo "ALL_DONE efficiency_profile $(date -Is)" | tee -a "$LOG_DIR/master.log"
