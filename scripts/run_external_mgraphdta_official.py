...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mgraphdta-repo", required=True)
    ...
    parser.add_argument("--block-num", type=int, default=3)
    parser.add_argument("--embedding-size", type=int, default=128)
    parser.add_argument("--filter-num", type=int, default=32)
...
def build_loaders(args: argparse.Namespace) -> tuple[dict[str, pd.DataFrame], dict[str, DataLoader]]:
    ...
    for split_name, split_frame in split_frames.items():
        data_list = frame_to_mgraphdta_data_list(split_frame, smile_graph=smile_graph)
...
def main() -> None:
    ...
    ModelClass = resolve_mgraphdta_model(args.mgraphdta_repo)
    ...
    if best_state is None:
        raise RuntimeError("MGraphDTA training produced no checkpoint.")
